...
def maxAvgTemps(data: List[String]): Set[(Int, Double)] = {
   val listing = data.map(el => el.split(" ").toList)
   val dropping = listing.map(el => el.drop(1))
   val grupping = dropping.map(el => el.zipWithIndex).flatten.map((el1, el2) => (el1.toDouble, el2+1))
   val grupping2 = grupping.groupBy(el => el(1))
   val helper = grupping2.map(el => (el._1, el._2.map(_._1.toDouble)))
   val maxTemps = helper.map { case (month, temps) =>
    (month, temps.max)
   }.toSet
